...
>
  <motion.div 
    whileHover={{ scale: 1.02 }}
    transition={{ type: "spring", stiffness: 300 }}
    className={'flex relative overflow-hidden dark:bg-white bg-black col-span-2 h-96 w-full rounded-2xl border-4 border-green-600 border-solid'}
  >
    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 0.9 }}
      transition={{ duration: 1, delay: 3.4 }}
      className={'bg-green-100 dark:bg-green-800 absolute w-full h-full opacity-90 dark:opacity-70 z-0'}
    ></motion.div>
    <div className={'h-full z-10 p-6 grid relative w-full items-stretch content-start overflow-hidden'}>
      <motion.div 
        ...
        className={'text-3xl font-bold text-black dark:text-white underline decoration-green-600 underline-offset-4'}
      >
        Visitors
      </motion.div>
      ...
      <motion.div 
        ...
        className={'absolute w-max text-[20rem] left-8 top-72 opacity-30 hover:opacity-20 transition-all ease duration-500'}
      >
