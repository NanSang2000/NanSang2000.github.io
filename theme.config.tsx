...
// 动态导入语言切换组件，避免服务端渲染时的localStorage错误
...
export const VercelLogo = (): JSX.Element => {
  ...
  const uniqueTextId = 'vercel-text-id';
  const uniquePathId = 'vercel-path-id';
    ...
    <div>
      ...
      >
        <svg height={20} viewBox="0 0 283 64" fill="none">
          <text id={uniqueTextId} x="10" y="20" fill="currentColor">Vercel</text>
          <path
            id={uniquePathId}
            fill="currentColor"
...
export const Logo = ({ height, color }: { height: number, color: string }): JSX.Element => {
    // Use stable IDs to avoid hydration mismatch
    const uniqueId = 'logo-path-id';
    const uniqueMaskId = 'logo-mask-id';
    const uniqueGroupId = 'logo-group-id';
        ...
        <div className={'flex flex-col lg:flex-row w-full'}>
          <div className={'flex flex-col w-full justify-between'}>
            ...
            <div className={'mt-3 font-sans font-thin'}>
              @{new Date().getFullYear()} Nan Sang. {t.rights}
            </div>
          </div>
          <div className={'h-full w-max flex justify-center items-center mt-3 lg:mt-0'}>
            <div
              className={'text-black dark:text-white w-max h-10 flex justify-center items-center mr-3 text-center whitespace-nowrap'}>{t.poweredBy}
            </div>
            <VercelLogo/>
          </div>
        </div>
